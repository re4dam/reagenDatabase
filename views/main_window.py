...
class MainWindow(QMainWindow):
    ...
    def _on_table_click(self, index: QModelIndex):
        ...
        row = index.row()
        self.current_selected_id = int(
            model.index(row, 0).data()
        )  # Get ID from first column
        name = model.index(row, 1).data()  # Get name from second column
        description = model.index(row, 2).data()  # Get description from third column

        # Fill the form with selected record data
